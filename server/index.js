  ...
  try {
    ...
    for (const img of img_data) {
      ...
      switch (imgSaveTypeInt) {
        ...
        case 3:
          const tempFilePath = alreadyFilePath_Dir_Path; // 임시 저장된 파일 경로
          const fileBuffer = fs.readFileSync(tempFilePath); // 파일 읽기
          const key = `${year}/${month}/${day}/${imgUUID}${path.extname(
            alreadyFileName
          )}`;
          // console.log("key (S3):", key);
          saveImageToS3(s3Config, bucketName, key, fileBuffer); // S3로 파일 업로드
          imgURL = key;
          break;

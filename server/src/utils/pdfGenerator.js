...
async function generatePDF(data) {
  ...
  const generatePage = (leftArray, rightArray, currentPage) => {
    ...
    const leftHtml = leftArray
      .map(
        (item) =>
          `<div class="left"><div class="contentWrapper"><div class="content"><span class="number">${item.question.id}</span>${item.question.content}</div></div></div>`
      )
      .join("");
    ...
    const rightHtml = rightArray
      .map(
        (item) =>
          `<div class="right"><div class="contentWrapper"><div class="content"><span class="number">${item.question.id}</span>${item.question.content}</div></div></div>`
      )
      .join("");
